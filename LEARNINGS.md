...
12. **What if the process crashes after `orderCreated` succeed, but before `publishOrderCreated` runs?**

**Answer**: The order itself is fine - the customer will get what he paid for. But the event that tell `notification-service` about it is gone forever and nothing revisits "the orders that exist but never published" on restart. The problem here is the orderCreated DB write and publishing event are not atomic operations, and they can not be made so because these are two different systems the operations are being performed on. The solution here is the `Outbox Pattern` - write the order and a durable "the order still needs to be published" record in the same atomic database transaction, so they can never disagree, then have a separate process retry publishing from that same durable record until it succeeds - a crash mid-flight just means the process picks it up again on restart, because the record survived even though our process didn't.


13. **What if the process crashes after `chargePayment` succeed, but before `orderCreated` runs?**

**Answer**: The Payment succeed but nothing downstream recorded it - is genuinely harder to trace and out of scope what "Outbox Pattern" solves. Real system handles it with durable "payment intent" records (write "We are about to charge X for order Y" before charging so a crash right after the payment leaves something to reconcile against) plus a periodic reconciliation job matching "charges that happened" against "orders that exist".


14. **Why does the saga need atomic operations at each individual step (like `reserveStock`'s conditional `findOneAndUpdate`) AND explicit compensating actions across steps?** What would break if we only had one of the two?

- **Atomicity within a step** (the findOneAndUpdate condition) protects against **other concurrent orders** interfering with this step — two different customers racing for the same last unit of stock, happening at the same moment, nothing to do with this order's own sequence.
- **Compensation across steps** protects against **this same order's own later step** failing after an earlier step of the same saga run already succeeded — sequential, not concurrent.

What breaks if you only had one: with compensation but no atomic reserve, two orders could both see "stock available" in the race window, both "succeed" at reserving (oversold), and no compensation logic would ever catch it — from the saga's own point of view, step 1 succeeded cleanly, so there's nothing to compensate. With atomic reserve but no compensation, overselling is prevented, but we're right back to Milestone 2's original deferred bug: payment fails, and reserved stock stays wrongly locked forever. They're solving two different failure shapes, not two intensities of the same one.
